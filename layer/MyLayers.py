...
class Multi_Attention2D(Layer):
    ...
    def call(self, x):
        ...
        q_prepare = tf.reshape(q_tensor, shape=(
        self.s_q[0] * self.s_q[1], self.s_q[2], self.s_q[3], self.s_q[4]))  # shape of (BS*q_T,H,W,q_dim)
        k_prepare = tf.reshape(k_tensor, shape=(
        self.s_k[0] * self.s_k[1], self.s_k[2], self.s_k[3], self.s_k[4]))  # shape of (BS*kv_T,H,W,k_dim)
        v_prepare = tf.reshape(v_tensor, shape=(
        self.s_v[0] * self.s_v[1], self.s_v[2], self.s_v[3], self.s_v[4]))  # shape of (BS*kv_T,H,W,v_dim)
        q = tf.nn.conv2d(input=q_prepare, filter=self.q_kernels, padding='SAME',
                         strides=[1, 1, 1, 1])  # +self.q_kernels_bias # shape of (BS*q_T,H,W,q_kernels_dim)
        k = tf.nn.conv2d(input=k_prepare, filter=self.k_kernels, padding='SAME',
                         strides=[1, 1, 1, 1])  # +self.k_kernels_bias # shape of (BS*kv_T,H,W,k_kernels_dim)
        v = tf.nn.conv2d(input=v_prepare, filter=self.v_kernels, padding='SAME',
                         strides=[1, 1, 1, 1])  # +self.v_kernels_bias # shape of (BS*kv_T,H,W,v_kernels_dim)
        q_relu = q  # tf.nn.relu(q)
        k_relu = k  # tf.nn.relu(k)
        v_relu = v  # tf.nn.relu(v)
        q_head_stacks = tf.split(q_relu, self.n_heads, axis=-1)
        q_restack = tf.reshape(tf.stack(q_head_stacks, axis=0),
                               shape=(self.n_heads * self.s_q[0], self.s_q[1], self.s_q[2] * self.s_q[3] * self.q_kdim))
        k_head_stacks = tf.split(k_relu, self.n_heads, axis=-1)
        k_restack = tf.reshape(tf.stack(k_head_stacks, axis=0),
                               shape=(self.n_heads * self.s_k[0], self.s_k[1], self.s_k[2] * self.s_k[3] * self.k_kdim))
        v_head_stacks = tf.split(v_relu, self.n_heads, axis=-1)
        v_restack = tf.reshape(tf.stack(v_head_stacks, axis=0),
                               shape=(self.n_heads * self.s_v[0], self.s_v[1], self.s_v[2] * self.s_v[3] * self.v_kdim))
        scores = tf.nn.softmax(tf.matmul(q_restack, tf.transpose(k_restack, (0, 2, 1))) / 8, axis=-1)  # TO MODIFY!
        z_t = tf.reshape(tf.matmul(scores, v_restack),
                         shape=(self.n_heads, self.s_v[0], self.s_q[1], self.s_v[2] * self.s_v[3] * self.v_kdim))
        z = tf.reshape(tf.transpose(z_t, (1, 2, 3, 0)),
                       shape=(self.s_v[0] * self.s_q[1], self.s_v[2], self.s_v[3], self.v_kernels_dim))

        out_t = tf.nn.conv2d(input=z, filter=self.out_kernels, padding='SAME',
                             strides=[1, 1, 1, 1])  # +self.out_kernels_bias
