...
class ManTraNet:
    ...
    def __init__(self, config_fileName=None):
        ...
        self.weight_file = "./pretrained_weights/mantranet/ManTraNet_Ptrain{}.h5".format(self.pretrain_index)
        #
    ...
    def get_Featex(self, trainable=False):
        ...
        Featex = mantranet.create_featex_vgg16_base(type_idx)
        Featex.trainable = trainable
        return Featex

    def get_original_model_0(self):
        model = mantranet.load_pretrain_model_by_index(0, self.manTraNet_modelDir)
        return model

    def get_original_model_by_id(self, modelId):
        assert modelId >= 0 and modelId < 5, "only 0 - 4 acceptable"
        model = mantranet.load_pretrain_model_by_index(modelId, self.manTraNet_modelDir)
        return model

    def get_original_model_4(self):
        model = mantranet.load_pretrain_model_by_index(4, self.manTraNet_modelDir)
        return model
    ...
    def get_original_model(self):
        ...
        devf5d = mantranet.NestedWindowAverageFeatExtrator(window_size_list=self.window_size_list, include_global=False,
                                                           # not add global, it's just a test!!!
                                                           output_mode='5d', minus_original=True,
                                                           name='nestedAvgFeatex_t')(bf)
        apply_normalization = True
        if (apply_normalization):
            sigma = mantranet.GlobalStd2D(name='glbStd_t')(bf)
    ...
    def get_model_1010_resize(self, layers_steps=[1, 3, 9, 27, 81], img_size=256):
        img_in = Input(shape=(None, None, 3), name='img_in')
        Featex = self.get_Featex(trainable=True)
        rf = Featex(img_in)
        resize = Lambda(lambda t: tf.image.resize_images(t, (img_size, img_size)), name='resize')(rf)
        rf = Conv2D(32, (1, 1), activation=None, use_bias=False, kernel_constraint=unit_norm(axis=-2),
    ...
    def get_model_1010_resize_visualization(self, layers_steps=[1, 3, 9, 27, 81], path="demo/"):
        ...
        t = rf
        for step in layers_steps:
            t = pa.PixelAttention(shift=step, useBN=False, useRes=True, useVisual=True,
                                  visualPath=path + "step_{}".format(step))(t)
...
def SPAN(lr=0.0002, input_size=256):
    model = ManTraNet().get_model_1010_resize(img_size=input_size)
